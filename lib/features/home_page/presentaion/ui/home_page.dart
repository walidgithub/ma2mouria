...
class _HomeViewState extends State<HomeView>
  ...
  List<String> permissionsList = ['admin', 'head', 'user'];
    ...
    return BlocConsumer<HomePageCubit, HomePageState>(
      listener: (context, state) async {
        if (state is GetRuleByEmailLoadingState) {
          ...
          await _appPreferences.removeUserData();
          ...
          // ------------------------------------------------------
        } else if (state is UpdateRuleLoadingState) {
          showLoading();
        } else if (state is UpdateRuleErrorState) {
          hideLoading();
          showAppSnackBar(
            context,
            state.errorMessage,
        ...
        } else if (state is UpdateRuleSuccessState) {
          hideLoading();
          // ------------------------------------------------------
        } else if (state is ResetRuleLoadingState) {
          showLoading();
        } else if (state is ResetRuleErrorState) {
          hideLoading();
          showAppSnackBar(
            context,
            state.errorMessage,
        ...
        } else if (state is ResetRuleSuccessState) {
          hideLoading();
          // ------------------------------------------------------
          ...
          spending = memberReportList.fold(
            0.0,
            (sum, m) => sum + double.parse(m.shareValue),
          );
                  ...
                  Checkbox(
                    ...
                    onChanged: (value) {
                      setState(() {
                        ...
                        HomePageCubit.get(
                          context,
                        ).getReceipts(getReceiptsRequest);
                      });
                    },
                  ),
                  Text(
                    ...
                    Checkbox(
                      ...
                      onChanged: (value) {
                        ...
                        if (showTotal) {
                          ...
                          HomePageCubit.get(
                            context,
                          ).getHeadReport(getHeadReportRequest);
                        } else {
                          MemberReportRequest memberReportRequest =
                              MemberReportRequest(name: userName, zone: zone);
                          HomePageCubit.get(
                            context,
      ...
      children: [
        ...
        Row(
          ...
          children: [
            Bounceable(
              onTap: () {
                if (selectedEmail == "" || selectedPermission == "" || selectedZone == "") {
                  return;
                }
                UpdateRuleRequest updateRuleRequest = UpdateRuleRequest(
                  email: selectedEmail,
                  rule: selectedPermission!,
                  zone: selectedZone!
                );
                HomePageCubit.get(context).updateRule(updateRuleRequest);
              },
              child: ClipRRect(
                borderRadius: BorderRadius.circular(20.r),
                child: BackdropFilter(
            ...
            Bounceable(
              onTap: () {
                ...
                ResetRuleRequest resetRuleRequest = ResetRuleRequest(
                  email: selectedEmail,
                );
                HomePageCubit.get(context).resetRule(resetRuleRequest);
              },
              child: ClipRRect(
                borderRadius: BorderRadius.circular(20.r),
                child: BackdropFilter(
